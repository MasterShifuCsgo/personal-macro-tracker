...
export default async function createDocument(model, data) {
  if (!isMongooseModel(model)) {
    throw new Error("createDocument(model, data): call does not have valid type for model parameter");
  }

  //check if foods is an array and has at least one element
  if (typeof data !== "object") {
    throw new Error("createDocument(model, data): call does not have valid type for data parameter");
  }
